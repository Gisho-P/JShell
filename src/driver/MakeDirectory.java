...
public class MakeDirectory implements Command {
	...
	/**
	 * Interprets the arguments and parses the of the command MakeDirectory,
	 * Will call exec and execute the command with the arguments if they are
	 * valid.
	 * 
	 * @param args
	 *            The list of arguments for MakeDirectory to use.
			...
			try {
				if ((slashIndex = i.indexOf("/")) == -1) {
					...
					if (splitPath.length != 0) {
						parentDir = FilePathInterpreter.interpretMakePath(
								s.getCurrentDir(), i);
						((Directory) parentDir).add(new Directory(
								splitPath[splitPath.length - 1]));
					} else
						message += "mkdir: cannot create a directory without a name\n";
				}
			} catch (Directory.NameExistsException e) {
				message += "mkdir: cannot create directory '" + i
						+ "': File exists\n";
