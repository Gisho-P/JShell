...
public class ExecuteHistoryTest {
	...
	public void testCallOutOfLowerBoundsNumber() {
		...
		assertEquals("out of bounds",
				"ERROR: Number entered is out of bounds.", s.getError());
	}
	...
	public void testCallCommandThatWillFailWhenCalledAgain() {
		JShell.commandProcessor("mkdir a", s);
		JShell.commandProcessor("mkdir a", s);
		assertEquals("Error for creating same dir should be called",
				s.getError(), "mkdir: cannot create directory 'a': File exists");
	}
	...
	public void testForceUsageError() {
		...
		assertEquals("Get the usage message", s.getError(),
				"! usage: !NUMBER > 0");
	}
}
