...
public class DisplayStoreString implements Command {
	...
	public String exec(List<String> args) {
		...
		try {
			...
			try {
				outputFile = (File) FilePathInterpreter.interpretPath(s.getCurrentDir(), (String) args.get(1));
			}
			// If the file doesn't exist create it
				...
				try {
					...
					if (((String) args.get(1)).contains("/")) {
						...
						outputFile = new File(((String) args.get(1)).substring(((String) args.get(1)).lastIndexOf("/") + 1));
						Directory fileDir = (Directory) FilePathInterpreter.interpretPath(s.getCurrentDir(), args.get(1).substring(0, args.get(1).lastIndexOf("/")));
